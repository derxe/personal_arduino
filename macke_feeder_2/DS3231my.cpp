...
//DS3231::DS3231() : _Wire(Wire) {
	...
	//Serial.println("NOrmal constructor");
//}

DS3231::DS3231(SoftwareWire & w) : _Wire(w) {
        //Serial.println("Constructor Software Wire");
}
...
DateTime RTClib::now(SoftwareWire & _Wire) {
  _Wire.beginTransmission(CLOCK_ADDRESS);
  _Wire.write((uint8_t) 0);	// This is the first register address (Seconds)
...
byte DS3231::getSecond() {
	...
	_Wire.requestFrom(CLOCK_ADDRESS, 1, true);
...
String DS3231::getDateStr(){
    bool century = false;
    bool h12Flag = false;
    bool pmFlag = false;


    uint8_t y = getYear();    // 0..99
    uint16_t year = 2000 + y;
    uint8_t mon = getMonth(century);
    uint8_t day = getDate();
    uint8_t hh  = getHour(h12Flag, pmFlag);
